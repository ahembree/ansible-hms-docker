...
To ensure no conflicting changes with an existing system, you can run this playbook in "check mode" (dry-run, preview, etc.) to see what, if any, changes would be made
...
6. <Badge text="Optional" variant="tip" /> If you will be installing to a remote host that is not the system you just cloned the repo to, see the [Remote Host Install](/other/remote-host) docs
...
For an example of a container map entry and the description of what each setting does, check the [Container List](/container-list#container-map-entry-example) docs
...
3. `hms_docker_media_share_type` : the type of network share (`cifs`, `nfs`, `local`)
    ...
    - See [the NAS setup page](/getting-started/nas) if using `nfs` or `cifs`
...
6. <Badge text="VPN Download Client" variant="success"/> If torrent download clients are enabled, see the [Download Clients and VPNs](/getting-started/vpn-and-dl-clients) documentation for the required variables

7. <Badge text="SSL/TLS Certificate" variant="success"/> If you want a SSL/TLS certificate for Traefik, see the **[SSL Certificates](/services/traefik/ssl)** docs.
...
See the [File Layout](/other/paths) documentation for info on where data is stored.


## Next Steps

Check out [Configuring DNS](/getting-started/dns) on how to setup the DNS records on a (recommended) internal DNS server.
...
If you already have DNS enabled and want to get up and running quicker, check out the [Automatic App Initialization](/getting-started/app-bootstrap).
