...
This playbook will also configure Docker to use the `172.27.0.0/16` IP range for all Docker networks and provision `/24` chunks for each Docker network from this block. If this overlaps with your internal range or if you wish to use a different range, modify the `hmsdocker_docker_network_base_subnet` variable in `inventory/group_vars/all/hmsd_advanced.yml`
