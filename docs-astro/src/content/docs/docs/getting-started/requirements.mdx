---
title: Requirements
description: "Minimum hardware, software, networking, and platform requirements for deploying HMS-Docker on Ubuntu, Raspberry Pi, and more."
sidebar:
  order: 1
---
...
- You own a domain name and are able to modify DNS `A` and `TXT` records (if you want SSL and/or dynamic DNS)
  - <Badge text="Preferred" variant="note" /> Have an internal DNS server that can resolve some or all of your domain name/zone.
    - A DNS server (such as PiHole or AdGuard Home) is not available in this project. However, if you use a UniFi Network controller, there is an optional [UniFi DNS role](/docs/getting-started/dns#unifi-dns-role) that can automatically manage DNS records for you.
