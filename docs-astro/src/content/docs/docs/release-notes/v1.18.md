...
Add HTTP/3 (QUIC over UDP) support to Traefik.

Set `traefik_http3_enabled: true` in `inventory/group_vars/all/traefik.yml` to enable it. This requires `traefik_ssl_enabled: true` since HTTP/3 only works over HTTPS.

When enabled:
- The `websecure` entrypoint advertises HTTP/3 support via the `Alt-Svc` header so compatible browsers can upgrade.
- The Traefik container exposes UDP port `443` in addition to TCP `443`. If you have a firewall in front of the host, ensure UDP/`443` is allowed.
