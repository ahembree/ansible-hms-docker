...
description: "Set up Cloudflare Tunnel to securely expose HMS-Docker services to the internet without port forwarding, with security considerations."
---
...
Traffic routed through Cloudflare Tunnel goes directly from the Tunnel container to the target container via the internal Docker network. It does **NOT** pass through Traefik, so no Traefik security controls (IP allowlists, middlewares) are applied.

It is your responsibility to understand and accept the risks and implement security controls where possible.
...
5. Follow [the steps here](https://developers.cloudflare.com/cloudflare-one/networks/connectors/cloudflare-tunnel/get-started/create-remote-tunnel/#2a-publish-an-application) to link containers to the tunnel, following the [container map](/docs/container-list#container-map) for the available container names and ports (use the container name as the "Service" name in the Cloudflare webgui, and append the port, e.g. `overseerr:5055`)
...
**SSO using Authentik will not work for containers accessed through the Tunnel** by default because Authentik authentication is enforced through Traefik middleware, and Tunnel traffic bypasses Traefik.

To use Authentik with Tunnel, you need to either forward ports 80/443 to your host (so Traefik handles the request) or follow the guide below.

:::

## Using Authentik through Cloudflare Tunnel

See the [Authentik docs for this project](/docs/services/authentik#authentik-protection-through-cloudflare-tunnel)
