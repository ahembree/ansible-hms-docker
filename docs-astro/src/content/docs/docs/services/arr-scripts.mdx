---
title: Arr-Scripts
description: "Enable Arr-Scripts integration for Radarr, Sonarr, Lidarr, and SABnzbd containers to add automated media management functionality."
---
...
To enable, enable the following:

```yaml
#inventory/group_vars/all/service_misc.yml
...
hmsdocker_svc_misc_arr_scripts_enabled: true
...
```
...
Container startup time will take longer after enabling and Traefik validations may fail, but it will not stop the playbook or containers from running.
...
If configuring a script that requires a URL, such as `plexUrl` or `tdarrUrl`, use the Container Name, such as `plex` or `tdarr`, as specified in the [Container Map](/docs/container-list#container-map)
