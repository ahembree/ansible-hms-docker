...
6. Configure an Application Outpost
    ...
    6. Replace the configuration in the Authentik webpage with this generated configuration, otherwise stuff will not work correctly.
...
This requires a large amount of work (creating Authentik Applications, Providers, and Outposts for _each individual enabled container_ for full protection) and is (in my personal opinion) not recommended.

I would recommend using [Tailscale](/docs/services/tailscale) as a VPN solution to access the services from only internally, but everyones use cases and environment are different. By proceeding, you understand the risks and potential impact.
