---
title: SSL
description: "Configure wildcard SSL certificates via Let's Encrypt and DNS validation for HMS-Docker services using Traefik as the reverse proxy."
---

## Generating Wildcard SSL Certificate

When SSL is enabled, the playbook generates a wildcard certificate (`*.example.com`) by default.
...
traefik_ssl_use_letsencrypt_staging_url: true # whether to use the Let's Encrypt staging URL for initial testing (default: `true`)
```
...
The certificate will say it is invalid within a browser, but if you check the issuer, it should come from the "Staging" server, meaning it worked successfully and you can then change this value to `false` to use the production server and get a valid certificate.
...
HTTP/3 is **enabled by default** whenever SSL is enabled. Traefik exposes UDP port `443` (in addition to TCP `443`) and advertises HTTP/3 to compatible browsers via the `Alt-Svc` response header — browsers that negotiate HTTP/3 will then connect over QUIC, and silently fall back to HTTP/2 over TCP if UDP/`443` is unreachable.

To opt out, set `traefik_http3_enabled: false` in `inventory/group_vars/all/traefik.yml`.

:::tip

For HTTP/3 to actually be reachable from the public internet, forward UDP/`443` on your router (in addition to TCP/`443`) and ensure your host firewall allows it. If UDP/`443` is blocked, the browser will keep working — it just won't use HTTP/3.
