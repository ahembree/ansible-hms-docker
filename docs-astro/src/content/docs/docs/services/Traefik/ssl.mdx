...
HTTP/3 is automatically active whenever SSL is enabled — no configuration required. Traefik exposes UDP port `443` (in addition to TCP `443`) and advertises HTTP/3 to compatible browsers via the `Alt-Svc` response header. Browsers that negotiate HTTP/3 connect over QUIC, and silently fall back to HTTP/2 over TCP if UDP/`443` is unreachable.
