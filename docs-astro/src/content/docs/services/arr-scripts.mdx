...
If configuring a script that requires a URL, such as `plexUrl` or `tdarrUrl`, use the Container Name, such as `plex` or `tdarr`, as specified in the [Container Map](/container-list#container-map)
